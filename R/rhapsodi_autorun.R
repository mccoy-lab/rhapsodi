...
#' @param outdir a string; default is "tmp/", might not be applicable
...
#' @param smooth_imputed_genotypes a bool; default is TRUE; used in `report_gametes` whether to use smoothed data from the HMM or original reads for ending filled gamete genotypes. If TRUE, doesn't replace smoothed data from HMM with original reads when there's a mismatch
#' @param smooth_crossovers a bool; default is FALSE; used in `report_gametes` whether to use smoothed data from the HMM or original reads for recombination finding. If TRUE, doesn't replace smoothed data from HMM with original reads when there's a mismatch
#'
