...
discover_meiotic_recombination <- function(original_gamete_data, complete_haplotypes, filled_gamete_data_list, positions, smooth_crossovers = TRUE, smooth_imputed_genotypes = FALSE, sampleName = "sampleT", chrom = "chrT", threads=2){
  ...
  if (smooth_crossovers){
    filled_gamete_forrecomb <- filled_gamete_data_list$filled_gametes_haps
  } else {
    if(smooth_imputed_genotypes){
      filled_gamete_forrecomb <- unsmooth(recode_gametes(original_gamete_data, complete_haplotypes), filled_gamete_data_list$filled_gametes_haps)
    } else{ filled_gamete_forrecomb <- filled_gamete_data_list$unsmoothed_gametes_haps}
  }
