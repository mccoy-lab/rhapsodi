...
#'
#' This function finds the number of false negatives when comparing by finding the locations for which nothing 
...
#' If there are no non-NA truths but some non-NA predictions, there are 0 false negatives.
#' However, if there are some non-NA truths, but no non_NA predictions, 
#' there are the same number of false negatives as there are non-NA truths because the predictions never predicted breakpoints for those truths 
#' 
#' 
...
#' @param cons a bool, default is FALSE; only applicable when multiple predictions intersect a single truth. if `cons` is TRUE, we take a conservative approach and consider all but one of these multiple intersection breakpoints as false negatives   
...
#' @param no_preds a bool, default is FALSE, if true, that means there were only NAs in the prediction information and no predicted recombination break points
#' 
